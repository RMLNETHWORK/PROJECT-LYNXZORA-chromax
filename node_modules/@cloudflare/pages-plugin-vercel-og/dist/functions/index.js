...
var responseIsValidHTML = (response) => response.ok && response.headers.get("Content-Type")?.includes("text/html");
var onRequestGet = async ({
  ...
  const match2 = url.pathname.match(`(.*)${escapeRegex(imagePathSuffix)}`);
  if (match2) {
    const props = {
      pathname: match2[1]
    };
    if (htmlRewriterHandlers) {
      const response = await next(match2[1]);
    ...
    return new HTMLRewriter().on("head", {
      element(element) {
        if (autoInject.openGraph) {
          element.append(
            `<meta property="og:image" content="${url.href}${imagePathSuffix}" /><meta property="og:image:height" content="${630}" /><meta property="og:image:width" content="${1200}" />`,
            { html: true }
          );
        }
      }
...
// ../.wrangler/tmp/pages-x6XWCz/functionsRoutes-0.8477190319509069.mjs
var routes = [
  {
    routePath: "/",
    mountPath: "/",
